...
vec3 phongLighting(vec3 V, vec3 N, vec3 L, float r)
{
    ...
    vec3 diffuse = u_kd*u_lightcol *d;
    

    // specular component           
    
   
    float specu = pow(max(0.0, dot(reflect(-L, N), V)), u_qs);
     vec3 specular = u_ks*u_lightcol * specu;
    // ambient component
...
void main()
{
    ...
    vec3 view_space = vec3(u_modelview *vec4(a_position, 1.0));
    vec3 n_vector = vec3(u_normalmat *vec4(a_normal, 1.0));
