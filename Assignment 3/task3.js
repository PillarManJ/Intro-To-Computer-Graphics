...
function main() {
    ...
    let camera = {
        viewMatrix: function () {
            ...
            cameramatrix = mult(cameramatrix, rotateX(rotx));

            cameramatrix = mult(cameramatrix, rotateY(roty));

            cameramatrix = mult(cameramatrix, rotateZ(rotup));

            cameramatrix = mult(cameramatrix, translate(0,0 , -dist)); 

            x = vec4(0, 0 , 0, 1); 

            x = mult(cameramatrix, x); 

            eye = x.splice(0,3);

            y = vec4(0,1,0,0);
            y = mult(cameramatrix, y); 
        ...
        projMatrix: function () {
            ...
            switch (document.getElementById("projectionSelect").value) {
                ...
                case "ortho":
                    ...
                    let bot = left; 
                    let top = right; 
                    let near_top = Number(-dist) + Number(near);
                    let far_top = Number(far); 

                    return ortho(left, right, bot, top, near_top, far_top);
    ...
    let materialManager = {
        ...
        materials: {
            ...
            "Polished Copper" : { colors:
                { "ka": [0.5, 0.0, 0.25] , "kd": [0.75, 0.75, 0.75], "ks": [0.75, 0.73, 0.79]},
                qs: 15,
            },

            "Pearl" : { colors:
                { "ka": [0.5, 0.5, 1] , "kd": [1, 0, 0], "ks": [0.9, 0.9, 0.9]},
                qs: 9
            },

            "Pewter" : { colors:
                { "ka": [0.51, 0.51, 0.51] , "kd": [0.75, 0.75, 0.75], "ks": [0.19, 0.9, 0.19]},
                qs: 6
            },
